...
- **Arreglo del empaquetado:** `packaging/` con el payload compartido (`payload.sh`, `files.sh`), los tres formatos (`deb.sh`, `rpm.sh` + `stella-client.spec.in`, `appimage.sh`) y `build.sh` como punto de entrada único; `VERSION` como fuente única de la versión (antes hardcodeada en `build-deb.sh`). El `.rpm` lleva `AutoReqProv: no` — sin eso rpmbuild pide los `.so` que el paquete lleva dentro y el RPM queda ininstalable — y declara **sólo por soname** (`libgtk-3.so.0()(64bit)`, `libwebkit2gtk-4.1.so.0()(64bit)`), que es el único nombre que direccionan igual Fedora, RHEL y openSUSE. El `.deb` usa alternativas (`libgtk-3-0t64 | libgtk-3-0`) para el renombrado de Ubuntu 24.04.
...
### 37. En Linux salían dos barras de título y el `.deb` "daba 404" — `hecho`
- **Síntoma 1:** la ventana abría con la barra del sistema **además** de la del launcher.
- **Dónde:** la creación de la ventana (`main.py`) y el backend que elige pywebview.
- **Causa:** en Wayland no hay forma de pedir una ventana sin decoración. El protocolo sólo permite elegir entre "la dibuja la aplicación" o "la dibuja el compositor", y GTK 3 no puede elegir ninguna de las dos a secas, así que KWin dibuja la suya encima de la del launcher. Medido: el objeto de la ventana es un `GdkWaylandWindow`, no dibuja barra propia (`decorated=False`, un solo hijo que ocupa todo) y aun así hay barra en pantalla; con el backend X11 el gestor de ventanas no añade nada (`_NET_FRAME_EXTENTS` **ausente**, frente a `4, 4, 30, 4` cuando sí hay decoración —esa medida, con `frameless=False`, es el control que confirma que la propiedad dice algo—).
- **Arreglo:** `main._avoid_wayland_frame()` pide el backend X11 cuando la sesión es Wayland **y** hay XWayland (`DISPLAY`), antes de importar pywebview; si alguien fija `GDK_BACKEND` a mano se respeta. De paso `main._use_gtk_backend()` fija `PYWEBVIEW_GUI=gtk`: pywebview se pasa a Qt en cuanto ve `KDE_FULL_SESSION`, y en un equipo con Qt instalado la ventana habría cambiado de motor —el icono, el portapapeles y `--check` son de GTK— además de dejar un volcado de error en el registro en cada arranque que parece un fallo y no lo es.
- **Verificado:** launcher real arrancado como desde el menú (`GDK_BACKEND` sin fijar): ventana maximizada, `_NET_FRAME_EXTENTS` ausente, **una sola barra** y la interfaz pintada (captura de la ventana por X11: 1920x1028, 8 116 colores distintos, franja de título oscura y contenido por debajo). `main.py --check` sale con código 0 y el registro ya no trae el volcado de Qt.
- **Síntoma 2 (el `.deb`):** al instalarlo en Ubuntu 24.04, `apt` fallaba con `404 Not Found` bajando `libglib2.0-0t64`, `libglib2.0-data`, `libglib2.0-bin`, `gir1.2-glib-2.0` y `libgirepository-2.0-0`.
- **Causa:** no era el paquete, era **la lista de paquetes del sistema, vieja**. Las URLs que pedía apuntan a la revisión `2.80.0-6ubuntu3.8`, que ya no está en el servidor (404 comprobado uno a uno), mientras el índice de `noble-security` ya publica la `3.9` (los cinco ficheros, descargables). `apt` resuelve con el índice que tiene en disco y no pregunta al servidor si sigue existiendo: con el índice antiguo pide ficheros que ya no están. Le pasaría a cualquier instalación en esa máquina, no sólo a ésta; se arregla con `sudo apt update` antes de instalar.
- **Comprobado que el paquete pide nombres reales:** cada dependencia de `build/depends-deb.txt` se contrastó contra los índices reales de Ubuntu 24.04, Ubuntu 22.04, Debian 12 y Debian 13. En 24.04 existen todas; en 22.04 y 12 no existen `libgirepository-2.0-0` ni el WebKitGTK 4.1, y eso es el límite ya declarado en el punto 35 (glib ≥ 2.80), no un nombre mal escrito. En el `.rpm` el problema no aplica: son requisitos por soname, y `dnf` refresca sus metadatos por su cuenta.
- **Verificado (los tres paquetes, 0.2.1):** `.deb` con `Version: 0.2.1` y el payload en `0.2.1`, extraído con `dpkg-deb -x` y ejecutado desde su enlace `/usr/bin/stella-client` → `--check` crea y cierra la ventana (código 0); `.rpm` con `0.2.1-1` y `Requires` por soname (`libgtk-3.so.0()(64bit)`, `libwebkit2gtk-4.1.so.0()(64bit)`, `libgirepository-2.0.so.0()(64bit)`…), extraído con `rpm2cpio|cpio` y las mismas pruebas; AppImage ejecutado con `--appimage-extract-and-run --check`.

### 38. El `.rpm` no se instalaba en Fedora: `dnf` decía «nada proporciona typelib(Gtk)» — `hecho`
- **Síntoma:** `sudo dnf install stella-client-0.2.1-1.x86_64.rpm` aborta con `Error al resolver la transacción: Problema: peticiones conflictivas` y una lista de `nada proporciona typelib(Atk) = 1.0`, `typelib(Gtk) = 3.0`, …, y también `libbz2.so.1.0()(64bit)`.
- **Causa 1 (la de fondo):** el `.rpm` pedía GTK y WebKitGTK por `typelib(...)`. Ese `Provides` lo genera **openSUSE** a partir de los ficheros `.typelib`; Fedora **no**, porque mete el typelib dentro del propio paquete de la librería (`gtk3` trae `Gtk-3.0.typelib`) sin publicarlo como provided. Es decir, el requisito «por typelib» que parecía universal sólo resolvía en openSUSE. Lo mismo con los nombres de paquete `gtk3` / `(webkit2gtk4.1 or …)`, que cambian por familia.
- **Causa 2 (el soname mal escrito):** `libbz2.so.1.0()(64bit)` sólo lo publican Debian y Arch. Fedora llama a esa librería `libbz2.so.1` (`bzip2-libs`), así que el requisito no resuelve allí. Y no hace falta pedirlo: el `libbz2` del payload es **transitivo** — el único ELF con `DT_NEEDED` externos es `_gi_cairo`, que sólo pide `libgobject`/`libcairo`/`libcairo-gobject`/`libc` — y lo arrastra el freetype nativo de cada distro.
- **Arreglo:** `packaging/audit_libs.py` pide GTK y WebKitGTK **por soname** (`libgtk-3.so.0()(64bit)`, `libgdk-3.so.0()(64bit)`, `libgdk_pixbuf-2.0.so.0()(64bit)`, `libpango-1.0.so.0()(64bit)`, `libatk-1.0.so.0()(64bit)`, `libharfbuzz.so.0()(64bit)`, `libwebkit2gtk-4.1.so.0()(64bit)`, `libjavascriptcoregtk-4.1.so.0()(64bit)`), que es lo que proveen las tres familias y además arrastra el typelib en cada una; y `RPM_SONAME_EXCEPTIONS` deja fuera `libbz2.so.1.0`. El `.deb` no cambia: allí sí hay nombres de paquete y `libbz2-1.0` es correcto.
- **Verificado:** `rpm -qp --requires` del `.rpm` regenerado da **34 entradas sin un solo `typelib(...)`, sin nombres de paquete y sin `libbz2.so.1.0`**; cada soname se contrastó contra los `Provides` reales de Fedora (`gtk3` → `libgtk-3.so.0()(64bit)`, `glib2` → `libgirepository-2.0.so.0()(64bit)` + typelibs GLib/GObject/Gio, `libbrotli`, `webkit2gtk4.1` → `libwebkit2gtk-4.1.so.0()(64bit)`, `bzip2-libs` → `libbz2.so.1()(64bit)`).
- **Límite:** sin un Fedora delante, la prueba final de la instalación la hace el usuario; aquí la comprobación es estática contra los `Provides` publicados.
